...
class EOBSobject():
    ...
    def cut_time(self, start, end, autosave = True, ext = False,
        ...
        if option == 'singleyear':
            for year in yU[np.where((yU >= start) & (yU <= end))]:
                ...
                src_time = np.ma.getdata(self.netcdf['time'][:])
                time = src_time[idx_time]
                res = {
    ...
    def save_netcdf(self, res = None, method = 'raw', readme = True):
        ...
        #Time
        src_time = self.netcdf['time']
        time = ds.createVariable('time', src_time.dtype, ('time',))
        for attr in src_time.ncattrs():
            setattr(time, attr, getattr(src_time, attr))
        #Day of the year
        ...
        y[:] = la
        time[:] = tout
        yearday[:] = np.arange(1, len(tout) + 1)
    ...
    def transf_eobstime(self, x, to = date(1950, 1, 1)):
        """
    ...
    def transf_eobsdate(self, x, number = False):
        ...
        """
        from datetime import date, timedelta
        start = date(1950, 1, 1) if self.info['for_swb2'] else date(1950, 1, 1)
